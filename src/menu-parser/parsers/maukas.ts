...
const parser: Parser = {
  ...
  async parse(url, lang) {
    ...
    return Array.from(document.querySelectorAll('.restaurant_menu'))
      .map((dayElement: any, i) => {
        const courseNames = Array.from(
          dayElement.querySelectorAll('.restaurant_menuitemname')
        );
        const courseProperties = dayElement.querySelectorAll(
          '.restaurant_menuitemdescription'
        );
        return {
          day: moment()
            .weekday(i + 1)
